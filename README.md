...
bash ./maintain-v7.sh --setup # Bootstrap on new machine (installs all tools) ./maintain-v7.sh # Interactive menu
Recommended Usage Cadence
Frequency	Actions	Menu Options
Weekly	Update, cleanup, temp clear, ClamAV No-PUA, rkhunter scan	3, 4, 5, 6b, 7a
...
SETUP
Option	Description
1	Initial Setup & Configuration Bootstrap (installs clamav, rkhunter, chkrootkit, aide, lynis, fail2ban, tmpreaper)
...
Recovery	Boot from Recovery Key (Option 98), run recover.sh, option 3
    ...
    ⚠️ Without the GRUB password, you cannot: modify GRUB boot parameters, boot into recovery mode, or change boot order in GRUB.
...
# Post-restore: sudo sysctl --system && sudo update-grub
Logs Location
...
AIDE Exclusions	Prevents false positives: /tmp/.mount_*, /var/log, /run, /dev, /proc, /sys, /mnt, /media
License
